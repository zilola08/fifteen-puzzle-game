...
function createBoard(gameSize) {
  ...
  // gameboard.addEventListener("dragleave",dragLeave);  //dragged image leaving anohter image
  // gameboard.addEventListener("drop",dragDrop);        //drag an image over another image, drop the image
...
class Puzzle {
  ...
  start(gameSize) {
    ...
    // this.emptyTileCoords = this.canMoveTile(this.emptyTile);
    ...
    for (let y = 0; y < this.rows; y++) {
      for (let x = 0; x < this.cols; x++) {
        ...
        if (tileIndex >= this.numberOfTiles) break;
        ...
        let emptyTile = this.tiles[this.tiles.length - 1];
        emptyTile.style.background = 'transparent';
        ...
        // this.tiles[this.tiles.length-1].removeEventListener('dragstart');
        // this.tiles[this.tiles.length-1].removeEventListener('dragover');
        // this.tiles[this.tiles.length-1].removeEventListener('dragenter');
  ...
  moveTile(tileIndex) {
    ...
    if (tileCoords != null) {
      ...
      this.positionTileAtCoord(this.tiles.length - 1,this.tileCoords[0],this.tileCoords[1]);
...
function dragStart(e) {
  e.dataTransfer.setData("text/plain",e.target.className);  //this refers to the img tile being dragged
...
function dragDrop(e) {
  ...
  console.log(data);
  let tileDragged = document.querySelector(`.${data}`);
  console.log(tileDragged)
  tileDragged.newPuzzle.moveTile(tileDragged);
